...
export function VetScribe() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Card com efeito de revelação 3D
      gsap.fromTo(
        cardRef.current,
        { opacity: 0, scale: 0.85, x: 80, rotateY: -15 },
        {
          opacity: 1,
          scale: 1,
          x: 0,
          ...
          <div className="relative z-10 max-w-3xl">
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              {["IA", "Veterinária", "Automação", "SOAP", "Templates"].map(tag => (
                <span key={tag} className="vet-tag px-3 py-1 bg-white/5 border border-white/10 rounded-md text-sm text-white/60">
                  {tag}
                </span>
              ))}
            </div>
